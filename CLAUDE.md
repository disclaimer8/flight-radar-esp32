...
- Tests (host, no hardware): `pio test -e native -f test_core` (63 cases)
- Companion app tests: `cd companion && flutter test` (54 cases)
...
badges, and fires emergency/military local notifications. **Military aircraft support** —
parsed from `dbFlags` in airplanes.live API, sent over BLE using flag bit `0x20`,
coloured `TFT_MAGENTA` on radar, and labeled with a `MIL` prefix in detail view.
